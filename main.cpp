                ...
                */
                class   Point
                {
                    ...
                    void    add(ffe_t x1, ffe_t y1, ffe_t x2, ffe_t y2, ffe_t & xR, ffe_t & yR) const
                    {
                        ...
                        // the additions
                        ffe_t s;                                                
                        if ( x1 == x2 && y1 == y2 )                    //if P and Q are same point
                        {
                            //2P                          
...
int main(int argc, char *argv[])
{
    ...
    // calulate all the points for this curve.
    myEllipticCurve.CalculatePoints();
    
    cout << "\nPoints on the curve (i.e. the group elements):\n";
    myEllipticCurve.PrintTable(cout,5);                //5 is number of columns in printing table
    cout << "\n\n";
    
    ec_t::Point P = myEllipticCurve[2];                //point at index 2 on curve
   ...
   // ec_t::Point Pk = a*Pb;
    ...
    int k = irand(1,myEllipticCurve.Degree()-1);
    ec_t::Point c1 = k*G;
    ec_t::Point Pk = k*Pb;
    ec_t::ffe_t c2x(Pk.x()+m.x());
    ec_t::ffe_t c2y(Pk.y()+m.y()) ;
    
    // encrypted message is: Pa,c1,c2
    cout << "Encrypted message from Alice to Bob = {Pa,c1,c2} = {" << Pa << ", " << c1 << ", (" << c2x <<","<<c2y<<") "<< "}\n\n";
    ...
    ec_t::Point cr = b*c1;
    ec_t::ffe_t m1d(c2x-cr.x());
    ec_t::ffe_t m2d (c2y-cr.y());
